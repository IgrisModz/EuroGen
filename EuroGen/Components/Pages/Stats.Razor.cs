...
public partial class Stats : IAsyncDisposable
{
	...
	static async Task<IEnumerable<Models.Stats>> GetStats(IEnumerable<Draw> draws, IEnumerable<Func<Draw, int>> selectors, int minYear, int maxYear, bool totalDraw = false)
	{
		List<Models.Stats> stats = [];

		var filteredDraws = draws.Where(d => d.DrawDate.Year >= minYear && d.DrawDate.Year <= maxYear).ToList();

		var valuesAndDates = filteredDraws.SelectMany(draw => draw.GetValuesAndDates(selectors)).ToList();
